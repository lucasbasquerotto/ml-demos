...
class TestEnvironment(unittest.TestCase):
    ...
    def test_initial_state(self):
        self.assertEqual(str(self.env.initial_state._expression), "x + 1")

    def test_step(self):
        action = Action(Expression("x + 1"), Expression("1 + x"))
        next_state, reward, terminated, truncated = self.env.step(action)
        self.assertEqual(str(next_state._expression), "1 + x")
    ...
    def test_reset(self):
        self.env.reset()
        self.assertEqual(str(self.env.current_state._expression), "x + 1")
